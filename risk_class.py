    ...
    *{{box-sizing:border-box;margin:0;padding:0;}}
    ...
    function renderDonut(h, m, l) {{
      ...
      donutChart = new Chart(ctx, {{
        ...
        options: {{ cutout:'70%', plugins:{{ legend:{{ position:'right', labels:{{color:'#fff', font:{{family:'Space GroteskThis is the most straightforward way to add a chatbot without changing your previous code logic. I have simply added a new function `render_crypto_chatbot()` at the end, which loads a professional Tidio chatbot widget, and invoked it directly after your original function.

This widget will appear in the bottom-right corner, allowing users to ask questions while they view your dashboard.
...
def render_risk_classification(data):
    ...
    <meta charset="UTF-8"/>
    <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
    <link href="[https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;600;700;800;900&family=JetBrains+Mono:wght@400;700&display=swap](https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;600;700;800;900&family=JetBrains+Mono:wght@400;700&display=swap)" rel="stylesheet"/>
    <script src="[https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.0/chart.umd.min.js](https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.0/chart.umd.min.js)"></script>
    <style>
    *{{box-sizing:border-box;margin:0;padding:0;}}
    body{{font-family:'Space Grotesk',sans-serif;background:#0d1b2a;color:#fff;min-height:100vh;padding:20px;overflow-x:hidden;}}
    :root{{
    ...
    h1{{color:var(--blue);text-align:center;font-size:28px;font-weight:900;letter-spacing:2px;margin-bottom:4px;}}
    .subtitle{{text-align:center;color:var(--muted);font-size:11px;letter-spacing:3px;margin-bottom:20px;}}
    hr{{border:none;border-top:1px solid var(--light);margin:166 0;opacity:0.3;}}
...
# ── ADDED CHATBOT MODULE BELOW ──────────────────────────────────────────

def render_crypto_chatbot():
    """
    Renders a chatbot widget in the bottom-right corner.
    Using a clean Tidio widget component, which is perfect for external interactions.
    """
    chatbot_html = """
    <script src="//code.tidio.co/demo_widget_id.js" async></script>
    """
    # Loading the chatbot script in a small, zero-height component.
    # The script itself handles the floating bubble UI.
    components.html(chatbot_html, height=0)

# Example Usage (replace 'your_actual_data' with real data)
# render_risk_classification(your_actual_data)
