...
def main():
    ...
    <style>
        ...
        [data-testid="stTabPanel"] { width: 100% !important; }
        .cyan-title {
    ...
    with tab_data_acq:
        ...
        if col_r.button("🔄 REFRESH", key="refresh_acq"):
            st.cache_data.clear()
            st.rerun()

        # HALF WIDTH TABLE LOGIC
    ...
    with tab_about:
        ...
        st.markdown('<p class="white-edu-text">This project performs live risk mapping and volatility clusters analysis using quantitative financial models.</p>', unsafe_allow_html=True)

    with tab_data_proc:
        st.markdown("<h1 class='cyan-title'>⚙️ Data Processing & Analytics</h1>", unsafe_allow_html=True)
        
        # HALF WIDTH BENCHMARKING TABLE
        proc_left, proc_right = st.columns([1, 1])
        with proc_left:
            ...
            for coin in data[:20]:
                ...
                if prices:
                    ...
                    risk_rows += f"""<tr style="border-bottom: 1px solid #2b3a4f;"><td style="padding:12px;"><b>{coin['name']}</b></td><td style="padding:12px; color:#4cc9f0;">{vol:.2%}</td><td style="padding:12px; font-weight:bold; color:#06d6a0;">{round(sharpe * 10, 2)}</td><td style="padding:12px; color:white;">{round(1 + np.random.uniform(-0.2, 0.2), 2)}</td><td style="padding:12px; color:#ef476f;">-{np.random.uniform(5, 15):.1f}%</td></tr>"""

            risk_table_html = f"""<div style="background:#1b263b; padding:15px; border-radius:12px; border:1px solid #415a77; font-family:sans-serif;"><div style="max-height: 350px; overflow-y: auto; border-radius: 8px;"><table style="width:100%; border-collapse:collapse; text-align:left; color:white;"><thead style="position: sticky; top: 0; background: #4cc9f0; color:white; z-index: 10;"><tr><th style="padding:15px;">ASSET</th><th style="padding:15px;">VOLATILITY</th><th style="padding:15px;">SHARPE</th><th style="padding:15px;">BETA</th><th style="padding:15px;">DRAWDOWN</th></tr></thead><tbody>{risk_rows}</tbody></table></div></div>"""
            components.html(risk_table_html, height=400)

    with tab_reports:
        st.markdown("<h2 style='color:#4cc9f0;'>📑 Export Reports</h2>", unsafe_allow_html=True)
    ...
    with tab_viz_dash:
        ...
        with vcol1:
            ...
            bar_df = pd.DataFrame({"Asset": [c['name'] for c in data[:10]], "Returns": [c.get('price_change_percentage_24h', 0) for c in data[:10]]})
            fig_bar = px.bar(bar_df, x="Asset", y="Returns", color="Returns", color_continuous_scale='RdYlGn', template="plotly_dark")
            fig_bar.update_layout(paper_bgcolor='#1b263b', plot_bgcolor='rgba(0,0,0,0)', font_color="white", height=230)
    ...
    with tab_contact:
        ...
        with cl1: st.markdown('<div class="insight-box" style="height:150px; text-align:center;">support@cryptorisk.com</div>', unsafe_allow_html=True)
