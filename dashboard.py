...
def main():
    ...
    with tab_data_acq:
        ...
        # --- LEFT SIDE CONTENT & TABLE ON RIGHT SIDE ---
        ...
        with t_col_left:
            ...
            <div class="insight-box" style="border-left-color: #06d6a0;">
                <span class="sync-icon">●</span> <b style="color:#06d6a0;">LIVE SYNCING:</b> Connected to V3 API<br>
                <b>Status:</b> Validating Data Integrity<br>
                <b>Last Update:</b> {datetime.now().strftime('%H:%M:%S')}<br>
                <b>Protocol:</b> Secure REST HTTPS
            </div>
            """, unsafe_allow_html=True)
            
            st.markdown(f"""
            <div class="insight-box" style="margin-top:20px; border-left-color:#ffd166;">
                <b style="color:#ffd166;">💡 Data Acquisition Note:</b><br>
                This module handles the ingestion of real-time market streams, 
                sanitizing raw JSON payloads for the calculation engine.
            </div>
            """, unsafe_allow_html=True)
    ...
    with tab_data_proc:
        ...
        # --- LEFT SIDE EXPLANATION & TABLE ON RIGHT SIDE ---
        b_col_left, b_col_right = st.columns([1, 1])
        
        with b_col_left:
            st.markdown("<div class='cyan-title'>🧠 Risk Calculation Model</div>", unsafe_allow_html=True)
            st.markdown("""
            <div class="insight-box" style="border-left-color: #ef476f;">
                <b style="color:#ef476f;">📉 Max Drawdown:</b><br>
                Measures the largest peak-to-trough decline before a new peak is achieved. Identifies the "Worst Case" historical risk.
            </div>
            <div class="insight-box" style="margin-top:20px; border-left-color: #4cc9f0;">
                <b style="color:#4cc9f0;">📈 Annualized Volatility:</b><br>
                Standard deviation of daily returns scaled to 365 days. [cite_start]"Price tells growth, Volatility tells risk". [cite: 80]
            </div>
            """, unsafe_allow_html=True)
            st.info("💡 Beta Analysis: Measures sensitivity relative to BTC price movement benchmark.")
    ...
    with tab_viz_dash:
        ...
        with filter_col1:
            selected_cryptos = st.multiselect("SELECT CRYPTOCURRENCIES TO COMPARE", options=[c['name'] for c in data], default=[data[0]['name'], data[1]['name']], key="sel_viz")
        with filter_col2:
        ...
        with scat_col:
            ...
            [cite_start]fig_bubble.update_layout(paper_bgcolor='#1b263b', plot_bgcolor='rgba(0,0,0,0)', font_color="white", height=400, xaxis=dict(title="Volatility (Risk) [cite: 83][cite_start]", gridcolor='#2b3a4f'), yaxis=dict(title="Average Returns [cite: 84]", gridcolor='#2b3a4f'))
            st.plotly_chart(fig_bubble, use_container_width=True)

        with table_col:
            [cite_start]st.markdown("<p style='color:#778da9; margin-bottom:10px;'>Strategic Quadrant Guide [cite: 87]</p>", unsafe_allow_html=True)
            [cite_start]class_html = """<div style="background:#1b263b; padding:15px; border-radius:12px; border:1px solid #415a77; font-family:sans-serif; color:white;"><table style="width:100%; border-collapse:collapse; text-align:left; font-size:13px;"><thead style="border-bottom:2px solid #4cc9f0;"><tr><th style="padding:8px;">QUADRANT</th><th style="padding:8px;">STRATEGY [cite: 24, 86][cite_start]</th></tr></thead><tbody><tr style="border-bottom:1px solid #2b3a4f;"><td style="padding:10px; color:#06d6a0; font-weight:bold;">Top-Left</td><td>Best Investment [cite: 88][cite_start]</td></tr><tr style="border-bottom:1px solid #2b3a4f;"><td style="padding:10px; color:#ef476f; font-weight:bold;">Bottom-Right</td><td>Worst Asset [cite: 89][cite_start]</td></tr><tr style="border-bottom:1px solid #2b3a4f;"><td style="padding:10px; color:#ffd166; font-weight:bold;">Top-Right</td><td>Speculative [cite: 90][cite_start]</td></tr><tr><td style="padding:10px; color:#4cc9f0; font-weight:bold;">Bottom-Left</td><td>Stable Asset [cite: 91]</td></tr></tbody></table></div>"""
